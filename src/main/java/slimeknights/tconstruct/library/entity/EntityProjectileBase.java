...
public abstract class EntityProjectileBase extends EntityArrow implements IEntityAdditionalSpawnData {
	...
	public boolean hasCapability(Capability<?> capability, EnumFacing facing) {
		return capability == CapabilityTinkerProjectile.PROJECTILE_CAPABILITY
	...
	public void onHitEntity(RayTraceResult raytraceResult) {
		...
		if (item.getItem() instanceof ToolCore && this.shootingEntity instanceof EntityLivingBase) {
			...
			ItemStack inventoryItem = AmmoHelper.getMatchingItemstackFromInventory(tinkerProjectile.getItemStack(),
					attacker, false);
				...
				if (item.getItem() instanceof IProjectile) {
					...
					if (launcher.getItem() instanceof ILauncher) {
						((ILauncher) launcher.getItem()).modifyProjectileAttributes(projectileAttributes,
								tinkerProjectile.getLaunchingStack(), tinkerProjectile.getItemStack(),
								tinkerProjectile.getPower());
					}
					...
					projectileAttributes.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(),
							new AttributeModifier(PROJECTILE_POWER_MODIFIER, "Weapon damage multiplier",
									tinkerProjectile.getPower() - 1f, 2));
		...
		if (bounceOff) {
			...
			// 1.9
			/*
			 * if (!this.worldObj.isRemote && this.motionX * this.motionX + this.motionY *
			 * this.motionY + this.motionZ * this.motionZ < 0.0010000000474974513D) { if
	...
	public void updateInAir() {
		...
		if (raytraceResult != null) {
			newPos = new Vec3d(raytraceResult.hitVec.xCoord, raytraceResult.hitVec.yCoord,
					raytraceResult.hitVec.zCoord);
		}
		// else
		// newPos = Vec3d.createVectorHelper(this.posX + this.motionX, this.posY +
	...
	public void drawCritParticles() {
		for (int k = 0; k < 4; ++k) {
			this.getEntityWorld().spawnParticle(EnumParticleTypes.CRIT, this.posX + this.motionX * (double) k / 4.0D,
	...
	/**
	 * Factor for the slowdown. 0 = no slowdown, >0 = (1-slowdown)*speed slowdown,
	 * <0 = speedup
	 */
	...
	/**
	 * Added to the y-velocity as gravitational pull. Otherwise stuff would simply
	 * float midair.
	 */
	...
	public void onCollideWithPlayer(@Nonnull EntityPlayer player) {
		if (!this.getEntityWorld().isRemote && this.inGround && this.arrowShake <= 0) {
			boolean pickedUp = this.pickupStatus == EntityArrow.PickupStatus.ALLOWED
	...
	public void writeSpawnData(ByteBuf data) {
		...
		// motion stuff. This has to be sent separately since MC seems to do hardcoded
		// stuff to arrows with this
