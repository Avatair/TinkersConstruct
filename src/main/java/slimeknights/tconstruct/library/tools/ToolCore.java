...
public abstract class ToolCore extends TinkersItem implements IToolStationDisplay {
	...
	public boolean onBlockStartBreak(ItemStack itemstack, BlockPos pos, EntityPlayer player) {
		...
		// the game can't detect that. so we have to switch around the items in the
		// hands for the break call
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(@Nonnull EntityEquipmentSlot slot,
		...
		if (slot == EntityEquipmentSlot.MAINHAND && !ToolHelper.isBroken(stack)) {
			multimap.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(), new AttributeModifier(ATTACK_DAMAGE_MODIFIER,
					"Weapon modifier", (double) ToolHelper.getActualAttack(stack), 0));
			multimap.put(SharedMonsterAttributes.ATTACK_SPEED.getName(), new AttributeModifier(ATTACK_SPEED_MODIFIER,
					"Weapon modifier", (double) ToolHelper.getActualAttackSpeed(stack) - 4d, 0));
		}
	...
	public String getItemStackDisplayName(@Nonnull ItemStack stack) {
		...
		List<Material> materials = TinkerUtil.getMaterialsFromTagList(TagUtil.getBaseMaterialsTagList(stack));
		// we save all the ones for the name in a set so we don't have the same material
		// in it twice
	...
	public static boolean isEqualTinkersItem(ItemStack item1, ItemStack item2) {
		...
		return TagUtil.getBaseMaterialsTagList(tag1).equals(TagUtil.getBaseMaterialsTagList(tag2)) && // materials used
				TagUtil.getBaseModifiersUsed(tag1) == TagUtil.getBaseModifiersUsed(tag2) && // number of free modifiers
																							// used
