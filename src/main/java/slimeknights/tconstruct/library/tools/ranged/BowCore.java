...
public abstract class BowCore extends ProjectileLauncherCore implements IAmmoUser, ILauncher {
	...
	public void playShootSound(float power, World world, EntityPlayer entityPlayer) {
		world.playSound(null, entityPlayer.posX, entityPlayer.posY, entityPlayer.posZ, SoundEvents.ENTITY_ARROW_SHOOT,
				SoundCategory.NEUTRAL, 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 1.2F) + power * 0.5F);
	}
	...
	public void modifyProjectileAttributes(Multimap<String, AttributeModifier> projectileAttributes,
		...
		if (projectileDamageModifier() != 0f) {
			projectileAttributes.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(), new AttributeModifier(
					LAUNCHER_DAMAGE_MODIFIER, "Launcher damage modifier", projectileDamageModifier() - 1f, 1));
		}
	}
