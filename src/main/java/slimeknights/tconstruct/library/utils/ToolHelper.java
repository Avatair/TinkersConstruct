...
public final class ToolHelper {
	...
	/**
	 * Returns the attack speed saved on the tool. This is normally just a number
	...
	public static boolean isToolEffective2(ItemStack stack, IBlockState state) {
		...
		// this will be the only place besides fortify where a modifier is hardcoded. I
		// promise. :L
		if (TinkerUtil.hasModifier(TagUtil.getTagSafe(stack), TinkerModifiers.modBlasting.getIdentifier())
	...
	public static ImmutableList<BlockPos> calcAOEBlocks(ItemStack stack, World world, EntityPlayer player,
		...
		// fire event
		TinkerToolEvent.ExtraBlockBreak event = TinkerToolEvent.ExtraBlockBreak.fireEvent(stack, player, state, width,
	...
	public static void breakExtraBlock(ItemStack stack, World world, EntityPlayer player, BlockPos pos,
		...
		if (!world.isRemote) {
			...
			if (block.removedByPlayer(state, world, pos, player, true)) // boolean is if block can be harvested, checked
																		// above
			{
				block.onBlockDestroyedByPlayer(world, pos, state);
		...
		if (block instanceof IShearable) {
			...
			if (target.isShearable(itemstack, world, pos)) {
				...
				for (ItemStack stack : drops) {
					...
					EntityItem entityitem = new EntityItem(player.getEntityWorld(), (double) pos.getX() + d,
				...
				// player.addStat(net.minecraft.stats.StatList.mineBlockStatArray[Block.getIdFromBlock(block)],
				// 1);
	 ...
	 */
	public static boolean attackEntity(ItemStack stack, ToolCore tool, EntityLivingBase attacker, Entity targetEntity,
			Entity projectileEntity, boolean applyCooldown) {
		// nothing to do, no target?
		if (targetEntity == null || !targetEntity.canBeAttackedWithItem() || targetEntity.hitByEntity(attacker)
		...
		if (hit && target != null) {
			...
			if (player != null) {
				...
				if (player.getEntityWorld() instanceof WorldServer && damageDealt > 2f) {
					...
					((WorldServer) player.getEntityWorld()).spawnParticle(EnumParticleTypes.DAMAGE_INDICATOR,
	...
	public static void swingItem(int speed, EntityLivingBase entity) {
		if (!entity.isSwingInProgress || entity.swingProgressInt >= 3 || entity.swingProgressInt < 0) {
			...
			if (entity.getEntityWorld() instanceof WorldServer) {
				((WorldServer) entity.getEntityWorld()).getEntityTracker().sendToTracking(entity,
						new SPacketAnimation(entity, 0));
			}
		}
	}
