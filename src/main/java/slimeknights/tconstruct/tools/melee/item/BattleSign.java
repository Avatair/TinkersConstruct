...
public class BattleSign extends TinkerToolCore {
	...
	public void reducedDamageBlocked(LivingHurtEvent event) {
		...
		if (event.getSource().isUnblockable() || event.getSource().isMagicDamage() || event.getSource().isExplosion()
				|| event.getSource().isProjectile() || event.isCanceled()) {
			return;
		}
		...
		// reduce damage. After this event the damage will be halved again because we're
		// blocking so we have to factor this in
		event.setAmount(event.getAmount() * 0.7f);

		// reflect damage
		if (event.getSource().getEntity() != null) {
			event.getSource().getEntity().attackEntityFrom(DamageSource.causeThornsDamage(player),
					event.getAmount() / 2f);
			damage = damage * 3 / 2;
		}
	...
	public void reflectProjectiles(LivingAttackEvent event) {
		...
		if (event.getSource().isUnblockable() || !event.getSource().isProjectile()
				|| event.getSource().getSourceOfDamage() == null) {
			return;
		}
		if (!shouldBlockDamage(event.getEntityLiving())) {
			return;
		...
		// ensure the player is looking at the projectile (aka not getting shot into the
		// back)
